...
#if defined(NDEBUG)
# error "Youcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf563da58afc0320bb0849393095f5e49f1ff21ea3e1b6f16935556df8c95f927");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Youcoin: starting difficulty is 1 / 2^12
...
{
    // Youcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Youcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Youcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Youcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Youcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Youcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2e8b2b70b5d27232d981ccff97bf1892b049ddc314ba9da0b648af0f36055b25");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Birth of YouCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0498cca66018f56d71056b6601362060cb40db5fac8a7b8cb2cd3b9dbd398eb62f35d38eb7e037d5af50cdf5db744c0e61b1af65b10a6484088e02a02b56727d7f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1532881948;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084907412;

        if (fTestNet)
        {
            block.nTime    = 1532881966;
            block.nNonce   = 386151866;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa112ce35f97431c9bbc5b7db67265c3222c906a42c40349e37b855aee3da6b5b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb1, 0xdc }; // Youcoin: increase each by adding 2 to bitcoin's value.
...
void static YoucoinMiner(CWallet *pwallet)
{
    printf("YoucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("youcoin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YoucoinMiner, pwallet));
}
